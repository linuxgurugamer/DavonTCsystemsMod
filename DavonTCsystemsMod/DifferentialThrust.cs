    ...
    public class DifferentialThrust : PartModule
    {
            ...
            foreach (Part p in EngineList)
            {
                ...
                foreach (PartModule pm in p.Modules)
                {
                    ...
                    if (pm.ClassName == "ModuleEngines")
                    {
                        ...
                        if (cModuleEngines.throttleLocked == false)
                        {
                            couldadd = true;
                        }
                        ;
                    ...
                    if (pm.ClassName == "ModuleEnginesFX")
                    {
                        ...
                        if (cModuleEnginesFX.throttleLocked == false)
                        {
                            couldadd = true;
                        }
                        ;
        ...
        void OnGUI()
        {
            if (visible && !paused)
            {
                ...
                //print("DifferentialThrust.OnGUI");
                if (drawguiMconfig)
                    windowPos = ClickThruBlocker.GUILayoutWindow(14132, windowPos, WindowGUIMconfig, "Module");

                if (drawguiTconfig)
                    windowPosT = ClickThruBlocker.GUILayoutWindow(4130, windowPosT, WindowGUITconfig, "Throttle 1-4");

                if (drawguiJconfig)
                    windowPosJ = ClickThruBlocker.GUILayoutWindow(23132, windowPosJ, WindowGUIJconfig, "Controls");

                if (drawguiSconfig)
                    windowPosS = ClickThruBlocker.GUILayoutWindow(14231, windowPosS, WindowGUISconfig, "Profile");

                if (drawguiCconfig)
                    windowPosC = ClickThruBlocker.GUILayoutWindow(14131, windowPosC, WindowGUICconfig, "Center Thrust");

                if (drawguiDconfig)
                    windowPosD = ClickThruBlocker.GUILayoutWindow(2131, windowPosD, WindowGUIDconfig, "Direction");
        ...
        public void Throttleconfig() { drawguiTconfig = true; }
        ...
        public void ThrottleExecute()
        {
            ...
            {
                Debug.Log("[Davon TC systems] unknown error: customized controls");
            }
        ...
        private void adjustInSim()
        {
            ...
            if (EngSelect != null)
            {
                if (thrustArea)
                {
                    ...
                    if (EngSelect.DifMod.aim <= EngSelect.aimlowest)
                    {
                        EngSelect.DifMod.aim = EngSelect.aimlowest;
                    }
                }
                else
                {
                    EngSelect.DifMod.aim = EngSelect.DifMod.aim + adjustmentSize;
                    if (EngSelect.DifMod.aim >= EngSelect.aimhighest)
                    {
                        EngSelect.DifMod.aim = EngSelect.aimhighest;
                    }
                }
                previousCoTX = CoTX;
                previousCoTY = CoTY;
        ...
        private float findCoTInSim(int direction)
        {
            ...
            foreach (simulatedEngine simE in simulatedEngineList)
            {
                ...
                {
                    if (simE.DifMod.CenterThrustMode == "ignore")
                    {
                        skip = true;
                    }
